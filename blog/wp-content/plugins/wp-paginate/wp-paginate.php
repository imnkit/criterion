		...
		function paginate($args = false) {
			...
			if ($pages > 1) {
				...
				if ($page > 1 && !empty($previouspage)) {
					...
					$output .= sprintf('<li><a href="%s" class="prev" title="Prev">%s</a></li>', $prevlink, stripslashes($previouspage));
				...
				if ($page < $pages && !empty($nextpage)) {
					...
					$output .= sprintf('<li><a href="%s" class="next" title="Next">%s</a></li>', $nextlink, stripslashes($nextpage));
