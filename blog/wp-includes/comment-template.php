...
function get_comment_reply_link( $args = array(), $comment = null, $post = null ) {
	...
	if ( get_option( 'comment_registration' ) && ! is_user_logged_in() ) {
		...
		$link = "<a title='Reply' class='comment-reply-link' href='" . esc_url( add_query_arg( 'replytocom', $comment->comment_ID ) ) . "#" . $respond_id . "' onclick='return addComment.moveForm(\"$add_below-$comment->comment_ID\", \"$comment->comment_ID\", \"$respond_id\", \"$post->ID\")'>$reply_text</a>";
...
function comment_form( $args = array(), $post_id = null ) {
	...
	$fields   =  array(
		...
		'author' => '<p class="comment-form-author">' . '' . __( '' ) . ( $req ? '' : '' ) . '' .

		            '<input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p>',

		'email'  => '<p class="comment-form-email">' . __( '' ) . ( $req ? ' ' : '' ) . ' ' .

		            '<input id="email" name="email" ' . ( $html5 ? 'type="email"' : 'type="text"' ) . ' value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></p>',

		'url'    => '<p class="comment-form-url">' . __( '' ) . ' ' .
	...
	$defaults = array(
		...
		'comment_field'        => '<p class="comment-form-comment">' . _x( '', 'noun' ) . ' <textarea id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p>',
				...
				<?php else : ?>
					...
					<form action="<?php echo site_url( '/wp-comments-post.php' ); ?>" method="post" id="<?php echo esc_attr( $args['id_form'] ); ?>" class="comment-form"<?php echo $html5 ? ' novalidate' : ''; ?>>
						...
						<p class="form-submit">
							...
							<input name="<?php echo esc_attr( $args['name_submit'] ); ?>" type="submit" id="<?php echo esc_attr( $args['id_submit'] ); ?>" value="<?php echo esc_attr( $args['label_submit'] ); ?>" title="Post Comment" />
